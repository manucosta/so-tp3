...
// Los comentarios que empiezan con muchas //////////// fueron pequeños tests que fuimos haciendo y los comentamos de esa forma.
void iniciar_eleccion(t_pid pid, int es_ultimo) {
  ...
  while(ack == 0) { // Este while va a estar ciclando mientras no le haya llegado el mensaje al siguiente en el anillo (se le acabó el timeout porque esta muerto, o bien porque estaba ocupado con otra cosa)
    ...
    if(respuesta[0] == 'O' && respuesta[1] == 'K') ack = 1; // Si llego el OK del ACK entonces pongo ack en true para salir del ciclo porque ya le pudo enviar a un proceso la tupla. DEBERIA CHECKEAR POR OK EN LUGAR DEL TIEMPO
  }
}
